...
export function useAdminUsers() {
  ...
  const mutation = useMutation({
    mutationFn: (input: { type: "active"; user: NativeUser; value: boolean } | { type: "role"; userId: string; value: NativeAppRole } | { type: "delete"; userId: string } | { type: "cancel"; pendingId: string } | { type: "create"; email: string; fullName: string; role: NativeAppRole; custodyItemsText?: string }) => input.type === "active" ? nativeAdminUsersContract.setActive(input.user, input.value) : input.type === "role" ? nativeAdminUsersContract.setRole(input.userId, input.value) : input.type === "delete" ? nativeAdminUsersContract.deleteManagedUser(input.userId) : input.type === "cancel" ? nativeAdminUsersContract.cancelPending(input.pendingId) : nativeAdminUsersContract.createPending({ email: input.email, fullName: input.fullName, role: input.role, custodyItemsText: input.custodyItemsText }),
