...
export default function OwnerDataResetScreen() {
  ...
  const resetApplication = async () => {
    ...
    try {
      ...
      console.log("🔥 سيرفر خطأ:", error); // رح يطبع الخطأ بالـ Terminal تبع إكسبو
      
      const errorMessage = error?.message || error?.details || JSON.stringify(error);
      Alert.alert("تفاصيل الخطأ الحقيقي", errorMessage);
    } finally {
      setSubmitting(false);
    }
      ...
      <ScrollView contentContainerClassName="p-4 pb-8" keyboardShouldPersistTaps="handled">
        <View className="rounded-3xl border border-[#F2B8B5] bg-[#FFF7F6] p-4">
          ...
          <View className="mt-5">
            ...
            className={`mt-4 h-12 flex-row items-center justify-center gap-2 rounded-2xl ${submitting ? "bg-[#D9A6A2]" : "bg-[#BA1A1A]"}`}
          >
            {submitting ? (
