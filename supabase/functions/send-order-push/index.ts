...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const messages = (tokens ?? []).map((row: { token: string }) => ({ to: row.token, sound: "new_order.mp3", title: "طلب جديد", body: `تم إسناد الطلب #${order.order_number} إليك`, data: { orderId: order.id, type: "assigned_order" }, priority: "high", channelId: "orders-v3" }));
    if (messages.length) {
