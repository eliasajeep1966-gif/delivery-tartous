-- Preserve manual assignment activity, but avoid a duplicate recent-activity item
-- when a captain is assigned immediately after a new order is created.
...
-- Keep the existing manual-assignment RPC unchanged for other admin workflows.
create or replace function private.assign_order_captain(
  p_order_id uuid,
  p_captain_id uuid
)
...
set search_path = ''
as $$
  select private.assign_order_captain(p_order_id, p_captain_id, true)
$$;

create or replace function public.assign_order_captain_without_activity(
  p_order_id uuid,
  p_captain_id uuid
)
returns public.orders
...
set search_path = ''
as $$
  select private.assign_order_captain(p_order_id, p_captain_id, false)
$$;

revoke all on function private.assign_order_captain(uuid, uuid, boolean) from public, anon;
grant execute on function private.assign_order_captain(uuid, uuid, boolean) to authenticated;
revoke all on function public.assign_order_captain_without_activity(uuid, uuid) from public, anon;
grant execute on function public.assign_order_captain_without_activity(uuid, uuid) to authenticated;
