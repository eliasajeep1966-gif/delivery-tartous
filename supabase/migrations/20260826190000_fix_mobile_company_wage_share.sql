...
drop function if exists public.get_captain_wage_period_summary(text, uuid, integer, date, uuid);
drop function if exists private.get_captain_wage_period_summary(text, uuid, integer, date, uuid);

create function private.get_captain_wage_period_summary(
...
begin
  ...
  grouped as (
    select
      ...
      coalesce(sum(lb.company_amount), 0)::numeric as company_total,
...
create function public.get_captain_wage_period_summary(
  p_period text default 'daily',
  p_captain_id uuid default null,
  p_limit integer default 100,
  p_before_period_start date default null,
