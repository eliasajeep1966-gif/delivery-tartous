-- Fix PL/pgSQL ambiguity between the RETURNS TABLE field expense_date
-- and the column selected by the page_days CTE.
...
begin
  ...
  page_days as (
    select eligible_days.expense_date
    from eligible_days
    order by eligible_days.expense_date desc
    limit p_day_limit
  ),
  page_meta as (
