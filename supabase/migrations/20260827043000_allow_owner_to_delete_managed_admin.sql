-- Delivery Tartous: the Owner may permanently delete a non-owner admin.
-- All other user-deletion role rules remain unchanged.
...
declare
  ...
  v_is_owner boolean := private.is_owner(auth.uid());
...
begin
  ...
  elsif v_target.role = 'admin'::public.app_role then
    if not v_is_owner then
      raise exception 'Only the Owner can delete an admin account' using errcode = '42501';
    end if;
  else
    raise exception 'Unsupported account role' using errcode = '42501';
  end if;
