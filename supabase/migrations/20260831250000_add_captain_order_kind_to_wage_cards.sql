-- Add order kind to captain wage rows for visual medicine/false-order cards.
-- Add order route labels to the captain wage response for the mobile wage cards.
...
begin
  ...
  if p_custom_date is not null then
    ...
    v_period_start := v_first_order_date + (((v_today - v_first_order_date) / 7)::integer * 7);
  ...
  with paid_by_ledger as (
    select cpi.financial_ledger_id, sum(cpi.captain_amount)::numeric as paid_amount
    ...
    select distinct on (cpi.financial_ledger_id)
      cpi.financial_ledger_id, cpi.payout_id, cp.paid_at
    from public.captain_payout_items cpi
  ...
  select jsonb_build_object(
    ...
    'rows', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'order_kind', page_rows.order_kind,
        'pickup_address', page_rows.pickup_address,
        'delivery_address', page_rows.delivery_address,
...
revoke all on function private.get_my_captain_wage_page(text, integer, integer, date) from public, anon;
grant execute on function private.get_my_captain_wage_page(text, integer, integer, date) to authenticated;
