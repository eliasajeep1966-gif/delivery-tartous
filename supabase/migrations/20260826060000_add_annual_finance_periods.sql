-- Delivery Tartous: add annual finance periods for mobile and web finance summaries.
-- Keeps Damascus business-day bucketing, existing permissions, and keyset pagination unchanged.
  ...
  periodized_ledger as (
    select
      case p_period
        ...
        when 'weekly' then date_trunc('week', ld.business_day::timestamp)::date
...
grant execute on function public.get_captain_wage_period_summary(text, uuid, integer, date, uuid)
  to authenticated;
revoke all on function private.get_company_profit_period_history(text, integer, date)
  from public, anon;
grant execute on function private.get_company_profit_period_history(text, integer, date)
  to authenticated;
