-- The server is authoritative: every treasury page contains at most 10 rows.
...
as $$
declare
  v_page_limit integer := least(greatest(coalesce(p_limit, 10), 1), 10);
begin
  ...
  select
    ...
    e.created_at,
    (select count(*) > v_page_limit from eligible)
  from eligible e
