-- Delivery Tartous: server-side captain wage pagination.
-- Returns only the requested ledger page together with complete totals for the selected Damascus business period.
...
set search_path = ''
as $$
  select private.get_my_captain_wage_page(p_period, p_limit, p_offset)
$$;

revoke all on function private.get_my_captain_wage_page(text, integer, integer)
  from public, anon;
grant execute on function private.get_my_captain_wage_page(text, integer, integer)
  to authenticated;
revoke all on function public.get_my_captain_wage_page(text, integer, integer)
  from public, anon;
grant execute on function public.get_my_captain_wage_page(text, integer, integer)
  to authenticated;
