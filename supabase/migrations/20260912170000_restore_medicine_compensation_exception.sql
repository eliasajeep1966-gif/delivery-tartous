...
begin
  ...
  if p_next_status in ('completed'::public.order_status, 'false_order'::public.order_status) then
    ...
    -- and allow the permanent "دواء" keyword to activate that same treatment.
    v_compensation := v_order.order_kind = 'medicine'
      or (
        v_exception is not null
        and lower(btrim(v_order.distribution_exception_keyword)) = 'دواء'
      );
