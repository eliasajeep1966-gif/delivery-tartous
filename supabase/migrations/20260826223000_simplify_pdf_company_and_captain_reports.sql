-- Delivery Tartous: concise PDF report summaries.
-- A period is optional. When it is omitted, the summary covers all recorded financial data.
...
create or replace function public.get_captain_report_range_summary(
  p_captain_id uuid,
  p_start_date date default null,
  p_end_date date default null
)
...
revoke all on function public.get_captain_report_range_summary(uuid, date, date)
  from public, anon, authenticated;
grant execute on function public.get_captain_report_range_summary(uuid, date, date)
  to authenticated;
