...
export default function Reports() {
  ...
  const isCaptainDetailsLoading = snapshot.captains.some((captain) => {
    const detailState = captainDetailsCache.get(captain.captainId);
    return !detailState || detailState.status === 'loading';
  });
  const captainDetailsErrors = snapshot.captains.filter((captain) => captainDetailsCache.get(captain.captainId)?.status === 'error');
  ...
  }, [loadCaptainDetails, snapshot.captains]);

  const retryReport = () => {
    void reload();
    snapshot.captains.forEach((captain) => {
      void loadCaptainDetails(captain.captainId, true);
    });
  };
