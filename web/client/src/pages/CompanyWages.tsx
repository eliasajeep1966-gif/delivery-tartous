...
export default function CompanyWages({ fullHistory = false }: { fullHistory?: boolean }) {
  ...
  const requestedCaptainIds = useRef(new Set<string>());
  ...
  useEffect(() => {
    if (!snapshot.captains.length) return;
    snapshot.captains.forEach((captain) => {
      const details = captainDetailsCache.get(captain.captainId);
      if (requestedCaptainIds.current.has(captain.captainId) || details?.status === 'loading' || details?.status === 'loaded') return;
      requestedCaptainIds.current.add(captain.captainId);
      void loadCaptainDetails(captain.captainId);
    });
  }, [captainDetailsCache, loadCaptainDetails, snapshot.captains]);

  useEffect(() => {
    if (!periodOptions.some((option) => option.key === periodKey)) setPeriodKey(periodOptions[0]?.key ?? '');
  }, [periodKey, periodOptions]);

  const rows = useMemo(() => filterFinanceRows(loadedRows, period, periodKey), [loadedRows, period, periodKey]);
  ...
  const visibleDailySummaries = fullHistory ? dailySummaries : dailySummaries.slice(0, 5);
  const isHistoryLoading = snapshot.captains.some((captain) => captainDetailsCache.get(captain.captainId)?.status === 'loading');
