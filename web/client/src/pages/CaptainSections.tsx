...
import { ArrowRight, CalendarDays, CheckCircle2, ClipboardList, Clock3, LoaderCircle, MapPin, Package, Phone, RefreshCw, ShieldCheck, UserRound, WalletCards } from 'lucide-react';
...
type CaptainWagePeriod = 'daily' | 'weekly' | 'monthly';
const captainWagePeriods: Array<{ key: CaptainWagePeriod; label: string }> = [{ key: 'daily', label: 'يومي' }, { key: 'weekly', label: 'أسبوعي' }, { key: 'monthly', label: 'شهري' }];
function isInCaptainWagePeriod(value: string, period: CaptainWagePeriod) {
  const timestamp = new Date(value).getTime();
  const now = new Date();
  const start = new Date(now);
  if (period === 'daily') start.setHours(0, 0, 0, 0);
  if (period === 'weekly') { start.setDate(now.getDate() - 6); start.setHours(0, 0, 0, 0); }
...
export function CaptainWages() {
  ...
  const [signingOut, setSigningOut] = useState(false);
  const [wagePeriod, setWagePeriod] = useState<CaptainWagePeriod>('daily');
  ...
  const visibleRows = useMemo(() => rows.filter((row) => isInCaptainWagePeriod(row.completed_at, wagePeriod)), [rows, wagePeriod]);
  const totals = useMemo(() => visibleRows.reduce((acc, row) => ({
    gross: acc.gross + row.gross_fee,
    captain: acc.captain + row.captain_amount,
    company: acc.company + (row.company_amount ?? (row.gross_fee - row.captain_amount)),
    paid: acc.paid + row.paid_amount,
    unpaid: acc.unpaid + row.unpaid_amount
  }), { gross: 0, captain: 0, company: 0, paid: 0, unpaid: 0 }), [visibleRows]);
  return <PageShell active="wages" onSignOut={() => void signOut()} signingOut={signingOut}><BackTitle title="أجوري" subtitle="تفاصيل أجورك وحالتها من السجلات الفعلية" />{loading ? <Loading /> : error ? <ErrorState message={error} onRetry={() => void load()} /> : <><div className="grid grid-cols-3 gap-1.5 rounded-2xl border border-[#dcecf4] bg-white p-1.5 shadow-[0_2px_8px_rgba(0,96,184,0.05)]">{captainWagePeriods.map((option) => <button key={option.key} type="button" onClick={() => setWagePeriod(option.key)} className={`h-10 rounded-xl text-xs font-extrabold transition-colors active:scale-[0.98] ${wagePeriod === option.key ? 'bg-[#086fc4] text-white shadow-[0_3px_8px_rgba(0,96,184,0.2)]' : 'text-[#5c7c90] hover:bg-[#eef8fd]'}`}>{option.label}</button>)}</div><div className="mt-3 grid grid-cols-2 gap-2.5"><Metric label="إجمالي الطلبات" value={money(totals.gross)} icon={<WalletCards size={16} />} /><Metric label="صافي الكابتن (70%)" value={money(totals.captain)} icon={<CheckCircle2 size={16} />} /><Metric label="حصة الشركة (30%)" value={money(totals.company)} icon={<WalletCards size={16} />} /><Metric label="المسدّد" value={money(totals.paid)} icon={<CheckCircle2 size={16} />} /><Metric label="المتبقي" value={money(totals.unpaid)} icon={<Clock3 size={16} />} /></div>{visibleRows.length === 0 ? <Empty icon={<WalletCards size={28} />} title="لا يوجد سجل أجور ضمن الفترة" body="ستظهر أجور الطلبات المكتملة هنا." /> : <div className="space-y-2">{visibleRows.map((row) => <article key={row.financial_ledger_id} className="rounded-2xl border border-white bg-white p-3.5 shadow-[0_2px_8px_rgba(0,96,184,0.05)]"><div className="flex items-start justify-between"><div><p className="text-[10px] text-[#748c9d]">الطلب #{row.order_number}</p><p className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-[#59788d]"><CalendarDays size={12} className="text-[#086fc4]" />التاريخ: {date(row.completed_at)}</p>{row.latest_paid_at && <p className="mt-1 text-[10px] text-[#7892a3]">آخر دفعة: {date(row.latest_paid_at)}</p>}</div><span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${row.is_fully_paid ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{row.is_fully_paid ? 'مسدّد' : 'غير مسدّد'}</span></div><div className="mt-3 grid grid-cols-3 gap-2 border-t border-dashed border-[#dcebf3] pt-2 text-center"><div><p className="text-[9px] text-[#7892a3]">الإجمالي</p><strong className="text-[11px] text-[#18547e]">{money(row.gross_fee)}</strong></div><div><p className="text-[9px] text-[#7892a3]">حصتك</p><strong className="text-[11px] text-[#086fc4]">{money(row.captain_amount)}</strong></div><div><p className="text-[9px] text-[#7892a3]">المتبقي</p><strong className="text-[11px] text-red-600">{money(row.unpaid_amount)}</strong></div></div></article>)}</div>}</>}</PageShell>;
}
