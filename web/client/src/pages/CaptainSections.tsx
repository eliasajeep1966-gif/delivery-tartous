...
export function CaptainWages() {
  ...
  const totals = useMemo(() => visibleRows.reduce((acc, row) => ({ gross: acc.gross + row.gross_fee, captain: acc.captain + row.captain_amount, paid: acc.paid + row.paid_amount, unpaid: acc.unpaid + row.unpaid_amount }), { gross: 0, captain: 0, paid: 0, unpaid: 0 }), [visibleRows]);
