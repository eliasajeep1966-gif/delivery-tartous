...
export default function CaptainWageDetail() {
  ...
  const totals = useMemo(() => ({
    ...
    companyTotal: (periodSummary?.grossTotal ?? rows.reduce((sum, row) => sum + row.grossFee, 0)) - (periodSummary?.captainNetTotal ?? rows.reduce((sum, row) => sum + row.captainAmount, 0)),
  ...
  const detailContent = captain ? <>
    <section className="mt-4 overflow-hidden rounded-2xl border border-[#d3e3f0] bg-white shadow-[0_2px_8px_rgba(0,72,141,0.05)]"><div className="flex items-center gap-3 bg-[#f8fbfe] p-4"><span className="grid h-12 w-12 place-items-center rounded-full bg-[#e5edf3] text-base font-bold text-[#53616f]">{captain.initial}</span><div className="min-w-0 flex-1"><h2 className="truncate text-[17px] font-bold">{captain.captainName}</h2><p className="mt-0.5 text-[11px] text-[#66727e]">{periodLabel || 'الفترة المختارة'} — {visibleRows.length} طلبات</p></div><span className="rounded-lg bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">كشف حي</span></div><div className="grid grid-cols-3 gap-px border-t border-[#e1ebf3] bg-[#e1ebf3]"><div className="bg-white p-3"><span className="text-[10px] font-bold text-[#66727e]">إجمالي الأجور</span><strong className="mt-1 block text-sm">{formatFinanceMoney(totals.grossTotal)}</strong></div><div className="bg-white p-3"><span className="text-[10px] font-bold text-emerald-700">صافي الكابتن (70%)</span><strong className="mt-1 block text-sm text-emerald-700">{formatFinanceMoney(totals.captainNetTotal)}</strong></div><div className="bg-white p-3"><span className="text-[10px] font-bold text-[#0060B8]">حصة الشركة (30%)</span><strong className="mt-1 block text-sm text-[#0060B8]">{formatFinanceMoney(totals.companyTotal)}</strong></div></div></section>
