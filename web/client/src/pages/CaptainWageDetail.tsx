...
export default function CaptainWageDetail() {
  ...
  const [orderStopsByOrderId, setOrderStopsByOrderId] = useState<Map<string, WebOrderStop[]>>(new Map());
  ...
  useEffect(() => {
    if (!captainRows.length) {
      setOrderStopsByOrderId(new Map());
      setStopsLoading(false);
      return;
    }
    let cancelled = false;
    setStopsLoading(true);
    void Promise.allSettled(captainRows.map(async (row) => [row.orderId, await webSupabase.reads.orderStops(row.orderId)] as const)).then((results) => {
      if (cancelled) return;
      const next = new Map<string, WebOrderStop[]>();
      results.forEach((result) => {
        if (result.status === 'fulfilled') next.set(result.value[0], result.value[1]);
      });
      setOrderStopsByOrderId(next);
      setStopsLoading(false);
    });
