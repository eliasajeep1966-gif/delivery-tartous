...
export default function CaptainHome() {
  ...
  const markFalseOrder = async () => {
    if (!currentOrder || currentStatus !== 'received' || isAdvancing || updatingOrderId === currentOrder.id) return;
    if (!window.confirm('هل تريد تسجيل هذا الطلب كطلب كاذب؟')) return;
    setIsAdvancing(true);
    try {
      const success = await transitionOrder(currentOrder.id, 'false_order');
      if (success) toast.success('تم تسجيل الطلب كطلب كاذب.');
    } finally {
      setIsAdvancing(false);
    }
  };
