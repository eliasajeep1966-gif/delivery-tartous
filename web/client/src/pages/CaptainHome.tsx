...
export default function CaptainHome() {
  ...
  const { availability, currentOrder, currentOrderStops, recentOrders, completedCount, completedGross, isInitialLoading, readError, updatingAvailability, updatingOrderId, reload, updateAvailability, transitionOrder } = useCaptainDashboard();
  ...
  return <div className="min-h-screen bg-[#edf8fd] text-[#17364d]" dir="rtl"><div className="captain-shell relative mx-auto min-h-screen w-full max-w-[453px] overflow-x-hidden bg-[linear-gradient(180deg,#f9fdff_0%,#e7f6fc_100%)] pb-24 shadow-[0_0_40px_rgba(0,72,141,0.08)]">
    ...
    <main className="space-y-3 px-3 pt-0 pb-6">
      ...
      {isInitialLoading ? <section className="flex min-h-40 items-center justify-center gap-2 rounded-2xl border border-[#dcecf4] bg-white text-sm font-bold text-[#086fc4]"><LoaderCircle className="animate-spin" size={19} />جارٍ تحميل حسابك...</section> : readError ? <section className="rounded-2xl border border-red-200 bg-red-50 p-4 text-center"><p className="text-sm font-bold text-[#ba1a1a]">{readError}</p><button type="button" onClick={() => void reload()} className="mx-auto mt-3 inline-flex h-10 items-center gap-2 rounded-xl border border-red-200 bg-white px-4 text-xs font-bold text-[#ba1a1a]"><RefreshCw size={15} />إعادة المحاولة</button></section> : <>
        ...
        <section className="overflow-hidden rounded-2xl border border-[#b9ddf1] bg-white shadow-[0_4px_14px_rgba(0,96,184,0.09)]"><div className="flex items-center gap-2 bg-[linear-gradient(110deg,#0f91d1,#0060b8)] px-3.5 py-2.5 text-white"><span className="grid h-7 w-7 place-items-center rounded-lg bg-white/15"><Truck size={16} /></span><div><h2 className="text-[13px] font-extrabold">الطلب الحالي</h2><p className="text-[9px] text-white/80">{currentOrder && currentStatus ? `الطلب #${currentOrder.order_number} · ${orderStatusPresentation[currentStatus].label}` : 'لا يوجد طلب نشط الآن'}</p></div></div>{currentOrder && currentStatus ? <div className="space-y-3 p-3.5"><div className="grid grid-cols-2 gap-2"><section className="rounded-xl border border-[#dcecf4] bg-[#f8fcfe] p-2.5"><h3 className="mb-2 flex items-center gap-1.5 text-[11px] font-extrabold text-[#285c79]"><PackageCheck size={14} className="text-[#086fc4]" />المصدر</h3><div className="space-y-1 text-[10px] text-[#54778d]"><p><b className="text-[#315f78]">الاسم:</b> {pickupStop?.contact_name || 'غير متاح'}</p><p><b className="text-[#315f78]">الرقم:</b> <a href={pickupStop?.contact_phone ? `tel:${pickupStop.contact_phone}` : undefined} dir="ltr" className="font-bold text-[#1478bf]">{pickupStop?.contact_phone || 'غير متاح'}</a></p><p><b className="text-[#315f78]">العنوان:</b> {pickupStop?.address || currentOrder.pickup_address}</p></div></section><section className="rounded-xl border border-[#dcecf4] bg-[#f8fcfe] p-2.5"><h3 className="mb-2 flex items-center gap-1.5 text-[11px] font-extrabold text-[#285c79]"><MapPin size={14} className="text-emerald-600" />الوجهة</h3><div className="space-y-1 text-[10px] text-[#54778d]"><p><b className="text-[#315f78]">الاسم:</b> {deliveryStop?.contact_name || currentOrder.customer_name}</p><p><b className="text-[#315f78]">الرقم:</b> <a href={deliveryStop?.contact_phone || currentOrder.customer_phone ? `tel:${deliveryStop?.contact_phone || currentOrder.customer_phone}` : undefined} dir="ltr" className="font-bold text-[#1478bf]">{deliveryStop?.contact_phone || currentOrder.customer_phone || 'غير متاح'}</a></p><p><b className="text-[#315f78]">العنوان:</b> {deliveryStop?.address || currentOrder.delivery_address}</p></div></section></div><OrderTimeline status={currentStatus} />{action ? <button type="button" disabled={isAdvancing || updatingOrderId === currentOrder.id} onClick={() => void advanceOrder()} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#086fc4] text-xs font-extrabold text-white shadow-[0_5px_12px_rgba(0,96,184,0.22)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60">{isAdvancing || updatingOrderId === currentOrder.id ? <LoaderCircle className="animate-spin" size={17} /> : <CheckCircle2 size={17} />}{isAdvancing ? 'جارٍ حفظ المرحلة...' : action.label}</button> : <div className="rounded-xl bg-emerald-50 px-3 py-2 text-center text-[11px] font-extrabold text-emerald-700">اكتمل الطلب ولا توجد خطوة إضافية.</div>}</div> : <div className="p-6 text-center"><PackageCheck className="mx-auto text-[#8fb2c7]" size={28} /><p className="mt-2 text-sm font-bold text-[#50778f]">لا يوجد طلب مسند إليك حالياً</p><p className="mt-1 text-[11px] text-[#7892a3]">فعّل التوفر ليتمكن المكتب من إسناد الطلبات.</p></div>}</section>
