...
export default function Wages(_: WagesProps) {
  ...
  const loadedRows = useMemo(() => Array.from(captainDetailsCache.values()).flatMap((details) => details.status === 'loaded' ? details.rows : []), [captainDetailsCache]);
  const periodOptions = useMemo(() => getFinancePeriodOptions(period, loadedRows), [period, loadedRows]);
  ...
  const periodRows = useMemo(
    () => filterFinanceRows(loadedRows, period, periodKey),
    [loadedRows, period, periodKey],
  );
  ...
  const captainLedgers = useMemo(() => snapshot.captains.map((captain) => {
    const details = captainDetailsCache.get(captain.captainId);
    if (details?.status !== 'loaded') return captain;
    const rows = filterFinanceRows(details.rows, period, periodKey);
    return {
      ...captain,
      rows,
  ...
  }), [captainDetailsCache, period, periodKey, snapshot.captains]);

  const changePeriod = (nextPeriod: FinancePeriod) => {
    setPeriod(nextPeriod);
    setPeriodKey(firstFinancePeriodKey(nextPeriod, loadedRows));
  };
  ...
  const registerCaptainPayout = async (captainId: string, unpaidTotal: number) => {
    ...
    try {
      ...
      if (expandedCaptainId === captainId) void loadCaptainDetails(captainId, true);
  ...
  const content = (
    <>
      <section className="mt-4"><WagePeriodControls period={period} periodKey={periodKey} rows={loadedRows} onPeriodChange={changePeriod} onPeriodKeyChange={setPeriodKey} /></section>
