...
import { type FinancePeriod } from '@/features/admin/financePeriod';
...
export default function Wages(_: WagesProps) {
  ...
  const { period, rows: periodRows, loading: isInitialLoading, error: readError, load } = useCaptainWagePeriodSummary();
  const [paymentInputs, setPaymentInputs] = useState<Record<string, string>>({});

  const periodKey = periodRows[0]?.periodStart ?? '';
  const periodLabel = periodRows[0] ? `${periodRows[0].periodStart} إلى ${periodRows[0].periodEnd}` : '';
  const periodTotals = useMemo(() => periodRows.reduce((totals, row) => ({
    captainNetTotal: totals.captainNetTotal + row.captainNetTotal,
    paidTotal: totals.paidTotal + row.paidTotal,
  }), { captainNetTotal: 0, paidTotal: 0 }), [periodRows]);
  const captainLedgers = periodRows;

  const changePeriod = (nextPeriod: FinancePeriod) => { void load(nextPeriod); };
  ...
  const content = (
    <>
      <section className="mt-4"><WagePeriodControls period={period} periodKey={periodKey} rows={[]} onPeriodChange={changePeriod} onPeriodKeyChange={() => undefined} /></section>
      <section className="mt-4 grid grid-cols-2 gap-3">
        <article className="rounded-2xl border border-emerald-100 bg-white p-3.5 shadow-[0_2px_8px_rgba(0,72,141,0.05)]"><span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><CircleDollarSign size={19} /></span><p className="mt-3 text-xs font-bold text-[#4f5d6b]">صافي الكباتن</p><strong className="mt-1 block text-[18px] text-emerald-700">{formatFinanceMoney(periodTotals.captainNetTotal)}</strong><span className="mt-1 text-[10px] text-emerald-700">من سجل الأجور الحي</span></article>
        <article className="rounded-2xl border border-amber-100 bg-white p-3.5 shadow-[0_2px_8px_rgba(0,72,141,0.05)]"><span className="grid h-9 w-9 place-items-center rounded-xl bg-amber-100 text-amber-700"><HandCoins size={19} /></span><p className="mt-3 text-xs font-bold text-[#4f5d6b]">دفعات مسلّمة</p><strong className="mt-1 block text-[18px] text-amber-700">{formatFinanceMoney(periodTotals.paidTotal)}</strong><span className="mt-1 text-[10px] text-amber-700">مسجلة في النظام</span></article>
      </section>
      ...
      <section className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-base font-bold">سجلات الكباتن — {periodLabel}</h2><span className="rounded-full bg-[#dbeeff] px-2.5 py-1 text-xs font-bold text-[#0060B8]">{periodRows.length} كباتن</span></div><div className="space-y-4">{captainLedgers.map((captain) => {
